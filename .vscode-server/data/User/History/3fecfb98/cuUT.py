...
def requests_made_recycler(request, pk = None):
    ...
    if request.method == 'GET': 
        recycler_consult = Request.objects.filter(recycler = pk, publication__state = False, is_active = True)
        recycler_count = Request.objects.filter(recycler = pk, publication__state = False, is_active = True).count()
        request_serializer = SearchSerializer(recycler_consult, many = True)
        if recycler_count == 0:
