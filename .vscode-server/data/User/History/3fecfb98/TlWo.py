...
def requests_availables(request, pk = None):
    ...
    if request.method == 'GET': 
        publication = Publication.objects.get(user_id = pk, state = True)
        for iterar in publication:
            publication =  Request.objects.filter(publication_id = pk, is_active = True)
        #print(Publication.objects.get(user_id = pk).first())
            request_serializer = SearchSerializer(publication, many = True)
