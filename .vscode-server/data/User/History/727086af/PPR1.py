...
class RequestAPIView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            print(request.data['recycler'])
            print(user.id)
            print(publication.id_publication)
            request = Request.objects.filter(recycler = user.id, publication = publication.id_publication).first()
            print(request)
            if (request):
