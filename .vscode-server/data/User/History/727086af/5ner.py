...
class RequestAPIView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            user = User.objects.filter(user_id = request.data['recycler']).first()
            publication = Publication.objects.filter(id_publication = request.data['publication']).first()
            request = Request.objects.filter(recycler = user.user_id, publication = publication.id_publication, is_active = True).first()
            if (request):
                 return Response({'message':'Ya ha hecho una solicitud para esta publicación'},status = status.HTTP_400_BAD_REQUEST)
            else: 
                if(publication.state == False):
                    serializer.save()
